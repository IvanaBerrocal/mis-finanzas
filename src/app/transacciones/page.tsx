...
import { Transaction, OWNER_LABELS } from "@/lib/types";
          ...
          {recent.length === 0 ? (
            ...
            <div className="space-y-3">
              {recent.map((t) => {
                ...
                const ownerBadge = t.owner === "mama"
                  ? "bg-pink-100 text-pink-700"
                  : "bg-teal-100 text-teal-700";
                const ownerLabel = t.owner ? OWNER_LABELS[t.owner] : "Propios";
                return (
                  <div key={t.id} className="flex items-center justify-between text-sm">
                    <div>
                      <div className="flex items-center gap-2">
                        <p className="font-medium text-gray-800">{t.category}</p>
                        <span className={`text-xs px-1.5 py-0.5 rounded-full font-medium ${ownerBadge}`}>
                          {ownerLabel}
                        </span>
                      </div>
